function [timeSeries, summaryStatistics] = do_ASrep...
...
%==========================================================================
 ...
 ASrep_o = cASrep(waveform, fs, config, descs);

 % Calculate descriptors from superclass methods
 for j=1:numel(descs)
     ASrep_o = ASrep_o.(descs{j});
 end

% Convert Representation Object to Struct 
repResults.(filename).(rep) = asObj_to_struct(ASrep_o);
clear ASrep_o
