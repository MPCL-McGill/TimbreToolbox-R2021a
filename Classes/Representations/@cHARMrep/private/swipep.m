% The swipep.m script by
% Camacho, A., Harris, J.G, (2008). 
% https://www.cise.ufl.edu/~acamacho/publications/swipep.m
...
for i = 1 : length(ws)
    ...
    % [ X, f, ti ] = specgram( xzp, ws(i), fs, w, o ); -------------> Initial Implementation
    [X, f, ti ] = spectrogram(xzp, w, o, ws(i), fs); % -------------> PROBLEM
    ti = ti.'; % ----------------------------------------------------> Not solved
    ti = ti - ti(1); % ----------------------------------------------> SOLVED
    % (ti now starts from zero: discrepancy between specgram VS spectrogram)
