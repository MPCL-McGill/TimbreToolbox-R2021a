function s = check_HARMconfig(userConfig)
%==========================================================================
% Checks the user supplied configurations for the Harmonic representation: 
% HARMrep 
...
s = cTTconfig.get_default_HARMconfig; 

acceptedFields = {'winType', 'winLength_s', 'winLength',...
    'hopSize_s', 'hopSize', 'magnitudeThreshold', 'minPartialsDuration',...
    'pitchRange', 'inharmonicityTolerance'};

acceptedWindows = {'hann', 'hamming', 'blackman','blackmanharris', ...
    'flattopwin', 'nutallwin', 'rectwin'};

% Check inharmonicityTolerance
if isfield(userConfig, 'inharmonicityTolerance')
    if (userConfig.inharmonicityTolerance >= 0) && (userConfig.inharmonicityTolerance <= 0.5)
        s.inharmonicityTolerance = userConfig.inharmonicityTolerance;
    else
        error('inharmonicityTolerance must be in the range of: [0, 0.5]');
    end
end

% Check pitchRange
if isfield(userConfig, 'pitchRange')
    if isequal(size(userConfig.pitchRange), [1 2]) 
        s.pitchRange = userConfig.pitchRange;
    else
        error('pitchRange should be specified as: [minFrequency, maxFrequency] .');
    end
end

% Check magnitudeThreshold
if isfield(userConfig, 'magnitudeThreshold')
    if userConfig.magnitudeThreshold <= 0
        s.magnitudeThreshold = userConfig.magnitudeThreshold;
    else
        error('magnitudeThreshold should be specified in dBFS.');
    end
end

% Check minPartialsDuration
if isfield(userConfig, 'minPartialsDuration')
    if userConfig.minPartialsDuration >= 0
        s.minPartialsDuration = userConfig.minPartialsDuration;
    else
        error('minPartialsDuration cannot be negative.')
    end
end
